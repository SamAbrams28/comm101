...
# basin_colors <- ('#---', '#---', '#---', '#---', '#---',)
ggplot(woa_sal, aes(basin, salinity, color = basin))+
  geom_boxplot() +
  theme_bw() +
...
ggplot(woa_sal, aes(latitude, salinity))+
  geom_point() +
  labs(x = "Latitude", y = "Salinity") +
  theme_bw()
ggsave('comm101p8.png')


# Code for when I have ocean basin data

# ggplot(woa_sal, aes(latitude, salinity, color = basin, shape = basin))+
# labs(x = "Latitude", y = "Salinity", color = "Ocean Basin")
# scale_color_manual (values = basin_colors)
